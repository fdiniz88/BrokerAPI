...
namespace Broker.Application.Customers.RegisterCustomer
{
    public class RegisterCustomerCommandHandler : ICommandHandler<RegisterCustomerCommand, CustomerDto>
    {
        ...
        {
            var customer = CustomerService.CreateRegistered(request.Email, request.Name, this._customerUniquenessChecker);
