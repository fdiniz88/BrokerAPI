...
namespace Broker.Domain.Customers.Orders
{
    public class OrderService : Entity
    {
        ...
        internal static OrderService CreateNew(List<OrderProductData> orderProductsData,
            List<ProductPriceData> allProductPrices,
            string currency,
            List<ConversionRate> conversionRates)
        {
            return new OrderService(orderProductsData, allProductPrices, currency, conversionRates);
        }
